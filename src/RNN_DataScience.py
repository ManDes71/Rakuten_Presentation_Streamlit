...
class DS_RNN(ds.DS_Model):
    ...
    def preprocess_text(self,text):
        ...
        tokens = word_tokenize(text, language='french' if lang not in ['english', 'spanish', 'german', 'dutch', 'italian'] else lang)
    ...
    def traiter_phrases(self,design,descrip):
        ...
        row = pd.Series({'phrases': chaine_designation, 'PAYS_LANGUE': chaine_pays_langue})
        X_test = self. preprossessing_X(row)
        
        print("X_test 1 = ",X_test)
        X_test= pd.Series(X_test['phrases'])
        ...
        print("type((X_train['phrases'])",type(X_train['phrases']))
        tokenizer.fit_on_texts(X_train['phrases'])
    ...
    def fit_modele(self,epochs,savefics=False,freeze=0,Train="None",stemming=False,lemming=False,spacy=False):
        ...
        model = self.create_modele(vocab_size)
...
class RNN_EMBEDDING(DS_RNN):     
     ...
     def create_modele(self,vocab_size=0):
        model = Sequential()
        print("self.EMBEDDING_DIM : ", self.EMBEDDING_DIM)
